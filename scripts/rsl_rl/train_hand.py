...
# ================= 新增：接收 WandB 数据集参数 =================
parser.add_argument("--dataset_artifact", type=str, default=None, help="WandB artifact path for the dataset (e.g., entity/registry/name:version)")
# ===============================================================
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # ================= 新增：动态拉取 WandB 数据集 =================
    if getattr(args_cli, "dataset_artifact", None):
        import wandb
        print(f"[INFO] 正在从 WandB 拉取数据集: {args_cli.dataset_artifact}")
        # 使用 Api() 可以在不启动额外 Run 的情况下静默下载数据
        api = wandb.Api()
        try:
            artifact = api.artifact(args_cli.dataset_artifact)
            downloaded_dir = artifact.download()
            print(f"✅ 数据集已就绪，位于本地缓存: {downloaded_dir}")
            
            # 动态覆盖环境配置中的数据集路径
            if hasattr(env_cfg, "commands") and hasattr(env_cfg.commands, "hand_tracking"):
                env_cfg.commands.hand_tracking.dataset_dir = downloaded_dir
                print(f"[INFO] 已动态覆盖 env_cfg.commands.hand_tracking.dataset_dir = {downloaded_dir}")
        except Exception as e:
            print(f"❌ 拉取 WandB 数据集失败，请检查名称、网络或权限: {e}")
            exit(1)
    # ===============================================================
