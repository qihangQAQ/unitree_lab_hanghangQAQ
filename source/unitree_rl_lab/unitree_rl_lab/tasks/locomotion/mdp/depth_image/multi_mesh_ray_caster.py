...
from .utils.mesh_utils import PRIMITIVE_MESH_TYPES, create_trimesh_from_geom_mesh, create_trimesh_from_geom_shape
from .utils.raycast_dynamic import raycast_dynamic_meshes
...
class MultiMeshRayCaster(RayCaster):
    ...
    mesh_views: ClassVar[dict[str, Any]] = {}
        ...
        for target_cfg in self._raycast_targets_cfg:
            ...
            for target_prim in target_prims:
                ...
                if target_cfg.is_shared and len(wp_mesh_ids) > 0:
                    ...
                    if curr_prim_base_path in self.meshes:
                        self.meshes[str(target_prim.GetPath())] = self.meshes[
                            curr_prim_base_path
                        ]
                # Reuse mesh imported by another ray-cast sensor (global cache).
                if str(target_prim.GetPath()) in self.meshes:
                    wp_mesh_ids.append(self.meshes[str(target_prim.GetPath())].id)
                ...
                if registered_idx != -1 and self.cfg.reference_meshes:
                    ...
                    self.meshes[str(target_prim.GetPath())] = wp_mesh
